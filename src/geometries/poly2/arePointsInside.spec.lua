...
local arePointsInside, create
do
	local ref = require("./init")
	arePointsInside, create = ref.arePointsInside, ref.create
end
test("poly2. arePointsInside() should return proper values", function()
	local polygon = create()
	local obs = arePointsInside({}, polygon)
	expect(obs).toBe(0)
	polygon = create({ { 1, 1 }, { 2, 2 }, { 3, 3 } })
	obs = arePointsInside({}, polygon)
	expect(obs).toBe(0)
	polygon = create({ { 0, 0 }, { 5, 0 }, { 5, 5 }, { 0, 5 } }) -- points overlapping points
	obs = arePointsInside({ { 0, 0 } }, polygon)
	expect(obs).toBe(0)
	obs = arePointsInside({ { 5, 0 } }, polygon) -- t.is(obs, 0) // FAILS
	obs = arePointsInside({ { 5, 5 } }, polygon)
	expect(obs).toBe(0)
	obs = arePointsInside({ { 0, 5 } }, polygon)
	expect(obs).toBe(0)
	obs = arePointsInside({ { 0, 0 }, { 5, 0 }, { 5, 5 }, { 0, 5 } }, polygon)
	expect(obs).toBe(0) -- points inside
