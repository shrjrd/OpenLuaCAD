...
test("vec4. equals() should return correct booleans", function()
	local vec0 = fromValues(0, 0, 0, 0)
	local vec1 = fromValues(0, 0, 0, 0)
	expect(equals(vec0, vec1)).toBe(true)
	local vec2 = fromValues(1, 1, 1, 1)
	expect(equals(vec0, vec2)).toBe(false)
	local vec3 = fromValues(0, 1, 1, 0)
	expect(equals(vec0, vec3)).toBe(false)
	local vec4 = fromValues(0, 0, 1, 1)
