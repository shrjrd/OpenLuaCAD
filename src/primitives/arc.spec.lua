...
local arc = require("./init").arc
test("arc (defaults)", function()
	local geometry = arc()
	local obs = path2.toPoints(geometry)
	expect(function()
		return path2.validate(geometry)
	end)["not"].toThrow()
	expect(#obs).toEqual(33)
end)
test("arc (options)", function()
	-- test center
	local exp = {
		{ 3, 2 },
	...
	local geometry = arc({ center = { 2, 2 }, segments = 16 })
	local obs = path2.toPoints(geometry)
	expect(function()
		return path2.validate(geometry)
	end)["not"].toThrow()
	expect(#obs).toEqual(17)
	...
	expect(comparePoints(obs, exp)).toBe(true) -- test makeTangent
	exp = {
		{ 1, 0 },
	...
	expect(#obs).toEqual(9)
	expect(comparePoints(obs, exp)).toBe(true)
end)
test("arc (rotations)", function()
	local exp = {
		{ 6.123233995736766e-17, 1 },
	...
	local geometry = arc({ startAngle = TAU / 4, endAngle = TAU / 2, segments = 16 })
	local obs = path2.toPoints(geometry)
	expect(function()
		return path2.validate(geometry)
	end)["not"].toThrow()
	...
	geometry = arc({
		startAngle = TAU * 0.75,
		endAngle = 270.000000005 * 0.017453292519943295,
		segments = 16,
	})
	obs = path2.toPoints(geometry)
	expect(function()
