...
local circle = require("./init").circle
test("circle (defaults)", function()
	local geometry = circle()
	local pts = geom2.toPoints(geometry)
	expect(function()
		return geom2.validate(geometry)
	end)["not"].toThrow()
	expect(#pts).toEqual(32)
end)
test("circle (options)", function()
	-- test center
	local geometry = circle({ radius = 3.5, center = { 6.5, 6.5 } })
	local pts = geom2.toPoints(geometry)
	local exp = {
		{ 10, 6.5 },
	...
	expect(#pts).toEqual(32)
	expect(comparePoints(pts, exp)).toBe(true) -- test radius
	...
	expect(comparePoints(pts, exp)).toBe(true) -- test startAngle
	...
	expect(#pts).toEqual(14)
	expect(comparePoints(pts, exp)).toBe(true) -- test endAngle
	...
	expect(comparePoints(pts, exp)).toBe(true) -- test full rotation with non-zero startAngle
	geometry = circle({ startAngle = 1, endAngle = 1 + TAU })
	pts = geom2.toPoints(geometry)
	expect(function()
		return geom2.validate(geometry)
	end)["not"].toThrow()
	expect(#pts).toEqual(32) -- test segments
	...
	expect(#pts).toEqual(5)
	expect(comparePoints(pts, exp)).toBe(true)
end)
test("circle (radius zero)", function()
	local geometry = circle({ radius = 0 })
	local pts = geom2.toPoints(geometry)
	expect(function()
		return geom2.validate(geometry)
	end)["not"].toThrow()
