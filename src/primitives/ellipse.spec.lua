...
local ellipse = require("./init").ellipse
test("ellipse (defaults)", function()
	local geometry = ellipse()
	local obs = geom2.toPoints(geometry)
	expect(function()
		return geom2.validate(geometry)
	end)["not"].toThrow()
	expect(#obs).toEqual(32)
end)
test("ellipse (options)", function()
	-- test center
	local geometry = ellipse({ center = { 3, 5 } })
	local obs = geom2.toPoints(geometry)
	local exp = {
		{ 4, 5 },
	...
	expect(#obs).toEqual(32)
	expect(comparePoints(obs, exp)).toBe(true) -- test radius
	...
	expect(#obs).toEqual(16)
	expect(comparePoints(obs, exp)).toBe(true) -- test startAngle
	...
	expect(#obs).toEqual(14)
	expect(comparePoints(obs, exp)).toBe(true) -- test endAngle
	...
	expect(#obs).toEqual(6)
	expect(comparePoints(obs, exp)).toBe(true) -- test full rotation with non-zero startAngle
	geometry = ellipse({ startAngle = 1, endAngle = 1 + TAU })
	obs = geom2.toPoints(geometry)
	expect(function()
		return geom2.validate(geometry)
	end)["not"].toThrow()
	expect(#obs).toEqual(32) -- test segments
	geometry = ellipse({ segments = 72 })
	obs = geom2.toPoints(geometry)
	expect(function()
		return geom2.validate(geometry)
	end)["not"].toThrow()
	expect(#obs).toEqual(72)
end)
test("ellipse (zero radius)", function()
	local geometry = ellipse({ radius = { 1, 0 } })
	local obs = geom2.toPoints(geometry)
	expect(function()
		return geom2.validate(geometry)
	end)["not"].toThrow()
